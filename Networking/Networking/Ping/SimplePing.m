...
- (instancetype)initWithAddress:(NSData *)address {
    ...
    self = [super init];
    if (self != nil) {
        self->_hostAddress = [address copy];
        self->_identifier  = (uint16_t) arc4random();
    }
    return self;
}
...
- (BOOL)validatePing4ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr identifier:(uint16_t *)identifierPtr {
    BOOL                result;
    ...
    if (icmpHeaderOffset != NSNotFound) {
        ...
        if (receivedChecksum == calculatedChecksum) {
            if ( (icmpPtr->type == ICMPv4TypeEchoReply) && (icmpPtr->code == 0) ) {
                *identifierPtr = OSSwapBigToHostInt16(icmpPtr->identifier);
...
- (BOOL)validatePing6ResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr identifier:(uint16_t *)identifierPtr {
    BOOL                    result;
...
- (BOOL)validatePingResponsePacket:(NSMutableData *)packet sequenceNumber:(uint16_t *)sequenceNumberPtr  identifier:(uint16_t *)identifierPtr {
    BOOL        result;
    
    switch (self.hostAddressFamily) {
        case AF_INET: {
            result = [self validatePing4ResponsePacket:packet sequenceNumber:sequenceNumberPtr identifier:identifierPtr];
        } break;
        case AF_INET6: {
            result = [self validatePing6ResponsePacket:packet sequenceNumber:sequenceNumberPtr identifier:identifierPtr];
        } break;
        default: {
            assert(NO);
            result = NO;
...
- (void)readData {
    ...
    if (bytesRead > 0) {
        ...
        strongDelegate = self.delegate;
        BOOL success = [self validatePingResponsePacket:packet sequenceNumber:&sequenceNumber identifier:&indentider];
        
        // CFSocketCreateWithNative has resuse mechanism.
        /*
         ...
         */
        // If we have muti simping obj, them read from same socket, we need to avoid this by identifier
        if (indentider != self.identifier) {
            return;
        }
